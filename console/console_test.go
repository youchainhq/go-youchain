...
func newTester(t *testing.T, confOverride func(*you.Config)) *tester {
	...
	nodeConfig := &node.Config{DataDir: workspace, Name: testInstance, NodeType: params.ArchiveNode.String()}
	stack, err := node.New(nodeConfig)
	if err != nil {
		t.Fatalf("failed to create node: %v", err)
	}
	youConf := you.NewDefaultConfig()
	youConf.Genesis = developGenesis(t)
	if confOverride != nil {
		confOverride(&youConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		keyStoreDir := filepath.Join(nodeConfig.DataDir, "keystore")
		logging.Info("keystore file", "keystore", keyStoreDir)
		ks, err := keystore.NewPassphraseKeyStore(keyStoreDir, keystore.StandardScryptN, keystore.StandardScryptP)
		youChain, err := you.New(&youConf, nodeConfig)
		accountManager, _ := accounts.NewManager(ks, youChain.BlockChain().CurrentBlock().Number())
		if err != nil {
			logging.Crit("accountManager failed", "err", err)
		}

		container := youapi.NewContainer(youChain, accountManager, stack)
		apis := youapi.GetAPIs(container)
		apis = append(apis, youChain.APIs()...)
		apis = append(apis, stack.APIs()...)
		stack.StartRPC(apis)
		return youChain, err
	}); err != nil {
...
func developGenesis(t *testing.T) *core.Genesis {
	g := &core.Genesis{}
	bs, err := ioutil.ReadFile("./testdata/genesis.json")
...
// the instance name, coinbase account, block number and supported
// console modules.
