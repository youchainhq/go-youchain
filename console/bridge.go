...
// RPC method (saved in jyou.newAccount) with it to actually execute the RPC call.
func (b *bridge) NewAccount(call otto.FunctionCall) (response otto.Value) {
	return b.newAccountOrValKey("jyou.newAccount", call)
}
...
func (b *bridge) OpenWallet(call otto.FunctionCall) (response otto.Value) {
	...
	val, err := call.Otto.Call("jyou.openWallet", nil, wallet, passwd)
	if err == nil {
		return val
	}
...
func (b *bridge) readPassphraseAndReopenWallet(call otto.FunctionCall) (otto.Value, error) {
	...
	return call.Otto.Call("jyou.openWallet", nil, wallet, passwd)
}
...
// original RPC method (saved in jyou.unlockAccount) with it to actually execute
// the RPC call.
func (b *bridge) UnlockAccount(call otto.FunctionCall) (response otto.Value) {
	...
	val, err := call.Otto.Call("jyou.unlockAccount", nil, account, passwd, duration)
	if err != nil {
		throwJSException(err.Error())
	}
	return val
...
// jyou.sign) with it to actually execute the RPC call.
func (b *bridge) Sign(call otto.FunctionCall) (response otto.Value) {
	...
	// Send the request to the backend and return
	val, err := call.Otto.Call("jyou.sign", nil, message, account, passwd)
	if err != nil {
		throwJSException(err.Error())
	}
	return val
...
func (b *bridge) SleepBlocks(call otto.FunctionCall) (response otto.Value) {
	...
	blockNumber := func() int64 {
		result, err := call.Otto.Run("you.blockNumber")
		if err != nil {
			throwJSException(err.Error())
		}
...
func (b *bridge) NewValKey(call otto.FunctionCall) (response otto.Value) {
	return b.newAccountOrValKey("jyou.newValKey", call)
}

func (b *bridge) UseValKey(call otto.FunctionCall) (response otto.Value) {
	var (
		addr   = call.Argument(0)
		passwd = call.Argument(1)
		keep   = call.Argument(2)
	)
	if !addr.IsString() {
		throwJSException("first argument must be the address of the validator key to use")
	}
	// if the password is not given or null ask the user and ensure password is a string
	if passwd.IsUndefined() || passwd.IsNull() {
		fmt.Fprintf(b.printer, "enter password for %s\n", addr)
		if input, err := b.prompter.PromptPassword("Passphrase: "); err != nil {
			throwJSException(err.Error())
		} else {
			passwd, _ = otto.ToValue(input)
	...
	if !passwd.IsString() {
		throwJSException("second argument must be the password to use the validator key")
	}
	if keep.IsUndefined() {
		if input, err := b.prompter.PromptInput("Keep the plaintext key for later reuse?(yes|no): "); err != nil {
			throwJSException(err.Error())
		} else {
			keep, _ = otto.ToValue(input)
		}
	}
	if keep.IsString() {
		s := keep.String()
		...
		case "yes", "Yes", "y", "Y":
			keep = otto.TrueValue()
		case "no", "No", "n", "N":
			keep = otto.FalseValue()
		}
		if !keep.IsBoolean() {
			k, err := strconv.ParseBool(s)
			if err == nil {
				keep, _ = otto.ToValue(k)
			}
		}
	}
	if !keep.IsBoolean() {
		throwJSException("third argument must be a boolean value ")
	}
	// Send the request to the backend and return
	val, err := call.Otto.Call("jyou.useValKey", nil, addr, passwd, keep)
	if err != nil {
		throwJSException(err.Error())
	}
	return val
...
func (b *bridge) newAccountOrValKey(source string, call otto.FunctionCall) (response otto.Value) {
	var (
		password string
		confirm  string
		err      error
	...
	ret, err := call.Otto.Call(source, nil, password)
	if err != nil {
		throwJSException(err.Error())
	}
	return ret
...
func throwJSException(msg interface{}) otto.Value {
	...
	if err != nil {
		logging.Error("Failed to serialize JavaScript exception", "exception", msg, "err", err)
	}
	panic(val)
}
