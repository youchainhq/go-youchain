...
func (mux *TypeMux) AsyncPost(ev interface{}) {
	go func() {
		err := mux.Post(ev)
		if err != nil {
			logging.Error("Post failed", "err", err)
		}
	}()
}
...
func (mux *TypeMux) tickDump() {
	...
	for {
		...
		case <-t.C:
			...
			logging.Trace("eventtick start")
			mux.mutex.RLock()
			for k, v := range mux.subm {
				for _, vv := range v {
					logging.Trace("eventtick", "k", k, "vv", vv.markC)
				}
			}
			mux.mutex.RUnlock()
			logging.Info("eventtick end")
		}
	}
}
