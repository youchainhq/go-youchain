...
import (
	...
	"encoding/binary"
	"errors"
	ycrypto "github.com/youchainhq/go-youchain/crypto"
...
func (k PrivateKey) Evaluate(m []byte) (index [32]byte, proof []byte) {
	...
	vrfData := elliptic.Marshal(curve, sHx, sHy) // 65 bytes.
...
func (pk *PublicKey) ProofToHash(m, proof []byte) (index [32]byte, err error) {
	...
	s := proof[0:32]
	t := proof[32:64]
	vrfData := proof[64 : 64+65]

	// uHx, uHy := elliptic.Unmarshal(curve, vrf)
	uHx, uHy := elliptic.Unmarshal(curve, vrfData) //////???
	if uHx == nil {
		return nilIndex, ErrInvalidVRF
	}
	...
	return sha256.Sum256(vrfData), nil
}
