...
const (
	...
	termTimeFormat  = "2006-01-02|15:04:05.000-0700"
	shortTimeFormat = "04:05.000"
	floatFormat     = 'f'
	termMsgJust     = 40
)
...
func ShortTerminalFormat(usecolor bool) Format {
	return FormatFunc(func(r *Record) []byte {
		var color = 0
		if usecolor {
			switch r.Lvl {
		...
		if atomic.LoadUint32(&locationEnabled) != 0 {
			...
			// Assemble and print the log heading
			if color > 0 {
				fmt.Fprintf(b, "\x1b[%dm%s\x1b[0m[%s] %s%s %s ", color, lvl, r.Time.Format(shortTimeFormat), location, padding, r.Msg)
			} else {
				fmt.Fprintf(b, "%s[%s] %s %s %s ", lvl, r.Time.Format(shortTimeFormat), location, padding, r.Msg)
			}
		} else {
			if color > 0 {
				fmt.Fprintf(b, "\x1b[%dm%s\x1b[0m[%s] %s ", color, lvl, r.Time.Format(shortTimeFormat), r.Msg)
			} else {
				fmt.Fprintf(b, "%s[%s] %s ", lvl, r.Time.Format(shortTimeFormat), r.Msg)
			}
		}
		// try to justify the log output for short messages
		length := utf8.RuneCountInString(r.Msg)
