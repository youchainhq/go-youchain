...
func makeTestTrie() (*Database, *Trie, map[string][]byte) {
	...
	triedb := NewDatabase(youdb.NewMemDatabase())
	trie, _ := New(common.Hash{}, triedb)

	// Fill it with some arbitrary data
	content := make(map[string][]byte) //for data check
	for i := byte(0); i < 255; i++ {
		// Map the same data under multiple keys
		k, _ := crypto.GenerateKey()
		key := crypto.PubkeyToAddress(k.PublicKey).Bytes()
		val := []byte{i}
...
func TestEmptySync(t *testing.T) {
	dbA := NewDatabase(youdb.NewMemDatabase())
	dbB := NewDatabase(youdb.NewMemDatabase())
	emptyA, _ := New(common.Hash{}, dbA)
	emptyB, _ := New(emptyRoot, dbB)

	for i, trie := range []*Trie{emptyA, emptyB} {
		if req := NewSync(trie.Hash(), youdb.NewMemDatabase(), nil).Missing(1); len(req) != 0 {
...
// makeChtTestTrie create a sample CHT test trie to test node-wise reconstruction.
func makeChtTestTrie() (*Database, *Trie, map[string][]byte) {
	// Create an empty trie
	db := youdb.NewMemDatabase()
	table := youdb.NewTable(db, "cht-")
	triedb := NewDatabase(table)
	trie, _ := New(common.Hash{}, triedb)
	...
	for i := 0; i < 128; i++ {
		...
		binary.BigEndian.PutUint64(encNumber[:], uint64(i))
		// blockNumber:blockHash
		trie.Update(encNumber[:], encNumber[:])
		content[string(encNumber[:])] = encNumber[:]
	}
	trie.Commit(nil)
	// Return the generated trie
	return triedb, trie, content
...
func TestIterativeSyncBatchedCht(t *testing.T) { testIterativeSyncCht(t, 100) }

func testIterativeSyncCht(t *testing.T, batch int) {
	// Create a random trie to copy
	srcDb, srcTrie, srcData := makeChtTestTrie()

	// Create a destination trie and sync with the scheduler
	db := youdb.NewMemDatabase()
	table := youdb.NewTable(db, "cht-")
	triedb := NewDatabase(table)
	sched := NewSync(srcTrie.Hash(), table, nil)
	...
	for len(queue) > 0 {
		...
		if index, err := sched.Commit(table); err != nil {
			t.Fatalf("failed to commit data #%d: %v", index, err)
		}
		queue = append(queue[:0], sched.Missing(batch)...)
	}
