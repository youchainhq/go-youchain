...
type Database struct {
	diskdb youdb.Database // Persistent storage for matured trie nodes
...
func NewDatabase(diskdb youdb.Database) *Database {
	return &Database{
		diskdb:    diskdb,
...
func (db *Database) DiskDB() youdb.Database {
	return db.diskdb
}
...
func (db *Database) Dereference(root common.Hash) {
	...
	if root == (common.Hash{}) {
		logging.Error("Attempted to dereference the trie cache meta root")
		return
	}
	db.lock.Lock()
	defer db.lock.Unlock()
	...
	logging.Debug("Dereferenced trie from memory database", "nodes", nodes-len(db.nodes), "size", storage-db.nodesSize, "time", time.Since(start),
...
func (db *Database) Cap(limit common.StorageSize) error {
	...
	if flushPreimages {
		for hash, preimage := range db.preimages {
			if err := batch.Put(db.secureKey(hash[:]), preimage); err != nil {
				logging.Error("Failed to commit preimage from trie database", "err", err)
				db.lock.RUnlock()
				return err
			}
	...
	for size > limit && oldest != (common.Hash{}) {
		...
		if batch.ValueSize() >= youdb.IdealBatchSize {
			if err := batch.Write(); err != nil {
				logging.Error("Failed to write flush list to disk", "err", err)
				db.lock.RUnlock()
				return err
			}
			batch.Reset()
	...
	logging.Debug("Persisted nodes from memory database", "nodes", nodes-len(db.nodes), "size", storage-db.nodesSize, "time", time.Since(start),
...
func (db *Database) Commit(node common.Hash, report bool) error {
	...
	if err := db.commit(node, batch); err != nil {
		logging.Error("Failed to commit trie from trie database", "err", err)
		db.lock.RUnlock()
		return err
	}
	// Write batch ready, unlock for readers during persistence
	if err := batch.Write(); err != nil {
		logging.Error("Failed to write trie to disk", "err", err)
		db.lock.RUnlock()
		return err
	}
	db.lock.RUnlock()
...
func (db *Database) commit(hash common.Hash, batch youdb.Batch) error {
