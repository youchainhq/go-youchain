...
func TestUintUint64LenEqual(t *testing.T) {
	tUInt := uint(20)
	tUInt64 := uint64(20)

	ba1, err := EncodeToBytes(tUInt)
	assert.Nil(t, err)
	ba2, err := EncodeToBytes(tUInt64)
	assert.Nil(t, err)

	assert.Equal(t, len(ba1), len(ba2))
}

func TestUint64Len(t *testing.T) {
	var uint64s = []uint64{1, 20, 50, 100, 127}

	for _, v := range uint64s {
		ba, err := EncodeToBytes(v)
		assert.Nil(t, err)
		assert.LessOrEqual(t, len(ba), 1)
	}
}
