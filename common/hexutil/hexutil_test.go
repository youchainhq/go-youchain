...
func TestUintToBytes(t *testing.T) {
	uintToBytesTest := []struct {
		...
		{i: 1, r: []byte{1}},
		{i: 0xff, r: []byte{0xff}},
		{i: 0x1122334455667788, r: []byte{0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88}},
		{i: 0x1100000000000000, r: []byte{0x11, 0, 0, 0, 0, 0, 0, 0}},
	}
	for _, test := range uintToBytesTest {
		enc := Uint64ToBytes(test.i)
		if !bytes.Equal(enc, test.r) {
			t.Errorf("input %x: wrong encoding %v", test.i, enc)
		}
	}
}
...
func BenchmarkEncodeFmt(b *testing.B) {
	input := make([]byte, 32)
	for i, _ := range input {
		input[i] = byte(i)
	}
