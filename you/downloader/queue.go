...
import (
	...
	log "github.com/youchainhq/go-youchain/logging"
)
...
type fetchRequest struct {
	...
	Count   int             // Requested chain element count (used for skeleton fills only)
...
// skeletonRange标识fillSkeleton业务中的请求的范围
type skeletonRange struct {
	From  uint64
	Count int
}
...
type queue struct {
	...
	headerTaskPool  map[uint64]*SkeletonHeader     // Pending header retrieval tasks, mapping starting indexes to skeleton header hashes
	headerTaskQueue *prque.Prque                   // Priority queue of the skeleton ranges to fetch the filling headers for
...
// up an CHT guaranteed header hash skeleton.
func (q *queue) ScheduleSkeleton(from uint64, skeleton []*SkeletonHeader) {
	q.lock.Lock()
	defer q.lock.Unlock()
	...
	q.headerTaskPool = make(map[uint64]*SkeletonHeader)
	q.headerTaskQueue = prque.New(nil)
	q.headerPeerMiss = make(map[string]map[uint64]struct{}) // Reset availability to correct invalid chains
	totalCount := skeleton[ln-1].Number - from + 1
	q.headerResults = make([]*types.Header, totalCount)
	q.headerProced = 0
	q.headerOffset = from
	q.headerContCh = make(chan bool, 1)

	// headers to be requested is as : [ from, ..., skeleton[0], ..., skeleton[ln-1] ]
	// the " from " is also needed to request
	for i := 0; i < ln; i++ {
		count := skeleton[i].Number - from + 1 // the skeleton[i] must be included
		q.headerTaskQueue.Push(&skeletonRange{From: from, Count: int(count)}, -int64(from))
		q.headerTaskPool[from] = skeleton[i]
		from = skeleton[i].Number + 1 //next from
	}
}
...
func (q *queue) Schedule(headers []*types.Header, from uint64) []*types.Header {
	...
	for _, header := range headers {
		...
		if q.mode == FastSync || q.mode == LightSync {
...
func (q *queue) ReserveHeaders(p *peerConnection, count int) *fetchRequest {
	...
	var send *skeletonRange
	skip := make([]*skeletonRange, 0)
	for send == nil && !q.headerTaskQueue.Empty() {
		item, _ := q.headerTaskQueue.Pop()
		sRange := item.(*skeletonRange)
		if q.headerPeerMiss[p.id] != nil {
			if _, ok := q.headerPeerMiss[p.id][sRange.From]; ok {
				skip = append(skip, sRange)
				continue
			}
		}
		send = sRange
	...
	for _, sRange := range skip {
		q.headerTaskQueue.Push(sRange, -int64(sRange.From))
	}
...
func (q *queue) cancel(request *fetchRequest, taskQueue *prque.Prque, pendPool map[string]*fetchRequest) {
	...
	if request.From > 0 {
		taskQueue.Push(&skeletonRange{request.From, request.Count}, -int64(request.From))
	}
	for _, header := range request.Headers {
		taskQueue.Push(header, -int64(header.Number.Uint64()))
	}
...
func (q *queue) DeliverHeaders(id string, headers []*types.Header, headerProcCh chan []*types.Header) (int, error) {
	...
	accepted := len(headers) == request.Count
	if accepted {
	...
	if !accepted {
		...
		q.headerTaskQueue.Push(&skeletonRange{request.From, request.Count}, -int64(request.From))
	...
	for q.headerProced+ready < len(q.headerResults) && q.headerResults[q.headerProced+ready] != nil {
		ready += 1 // Because the request count is not constant, so we check here step by step.
	}
	if ready > 0 {
		...
		case headerProcCh <- process:
			...
			lastProced := q.headerProced
			q.headerProced += len(process)
			// Dereference the processed headers, so that they can be released by the GC as soon as possible.
			for i := lastProced; i < q.headerProced; i++ {
				q.headerResults[i] = nil
			}
		default:
		}
	}
