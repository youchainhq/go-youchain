...
func init() {
	var r enr.Record
	//set the default node nat and nodetype configuration:
	r.Set(enr.NAT(1))
	r.Set(enr.NODETYPE(2))
	r.Set(enr.IP{0, 0, 0, 0})
	nullNode = enode.SignNull(&r, enode.ID{})
}

//add local node nat and nodetype config as default testTable
...
// nodeAtDistanceWithNATandNodetype for enode.LogDist(base, n.id) == ld.
func nodeAtDistanceWithNATandNodetype(base enode.ID, ld int, ip net.IP, nat, nodetype uint16) *node {
	var r enr.Record
	r.Set(enr.IP(ip))
...
//newPingRecorderWithNodetype takes nodetype input
func newPingRecorderWithNodetype(nodetype uint16) *pingRecorder {
	var r enr.Record
	r.Set(enr.IP{0, 0, 0, 0})
	//set the default node nat and nodetype configuration:
	r.Set(enr.NAT(1))
