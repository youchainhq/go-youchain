...
//@matt: Test addThroughPing for the PING sender as if it is under the connection rule
func TestTable_AddThroughPing(t *testing.T) {
	transport := newPingRecorderWithNodetype(2)
	...
	pingSender := wrapNode(enode.NewV4(&pingKey.PublicKey, net.IP{172, 0, 1, 1}, 99, 99, uint16(6), uint16(1)))
	last := fillBucket(tab, pingSender)
	t.Logf("last node id is: %s\n", last.ID())
	...
	found := contains(tab.bucket(pingSender.ID()).replacements, pingSender.ID())
	t.Logf("sender is in the bucket`s replacement: %t\n", found)
...
func testPingReplace(t *testing.T, newNodeIsResponding, lastInBucketIsResponding bool) {
	...
	// Fill up the sender's bucket.
	pingKey, _ := crypto.HexToECDSA("45a915e4d060149eb4365960e6a7a45f334393093061116b197e3240065ff2d8")
	pingSender := wrapNode(enode.NewV4(&pingKey.PublicKey, net.IP{}, 99, 99, uint16(5), uint16(2)))
	last := fillBucket(tab, pingSender)
...
func TestTable_IPLimit(t *testing.T) {
	...
	defer tab.Close()
	defer db.Close()
	t.Logf("nat is: %d, node type is: %d\n", tab.self().NAT(), tab.self().Nodetype())
	for i := 0; i < tableIPLimit+1; i++ {
		n := nodeAtDistanceWithNATandNodetype(tab.self().ID(), i, net.IP{172, byte(i), byte(i), byte(i)}, 1, 2)
		tab.add(n)
	}
	//@matt: // IP address limits per bucket and table
	//	bucketIPLimit, bucketSubnet = 2, 24 // at most 2 addresses from the same /24
	//	tableIPLimit, tableSubnet   = 10, 24
	t.Logf("table length is:%d\n", tab.len())
...
func TestTable_closest(t *testing.T) {
	...
	test := func(test *closeTest) bool {
		...
		result := tab.closest(test.Target, test.N, tab.self().NAT(), tab.self().Nodetype()).entries
...
func TestTable_ReadRandomNodesGetAll(t *testing.T) {
	...
	test := func(buf []*enode.Node) bool {
		...
		for i := 0; i < len(buf); i++ {
			...
			tab.stuff([]*node{nodeAtDistanceWithNATandNodetype(tab.self().ID(), ld, intIP(ld), 1, 2)})
		}
		gotN := tab.ReadRandomNodes(buf)
		t.Logf("length of buf %d, gotN: %d\n", len(buf), gotN)
		if gotN != tab.len() {
...
func TestTable_Lookup(t *testing.T) {
	...
	seed := wrapNode(enode.NewV4(seedKey, net.IP{}, 0, 256, 1, 2))
	tab.stuff([]*node{seed})
...
func (tn *preminedTestnet) findnode(toid enode.ID, toaddr *net.UDPAddr, target encPubkey) ([]*node, error) {
	...
	for i, ekey := range tn.dists[toaddr.Port] {
		...
		node := wrapNode(enode.NewV4(key, net.ParseIP("127.0.0.1"), i, next, 1, 2))
