...
const (
	...
	maxPacketSize = 1280 // Udp reads the buffer size
)
...
func makeEndpoint(addr *net.UDPAddr, rudpPort uint16, nat enode.NATType, nodetype params.NodeType) rpcEndpoint {
	ip := net.IP{}
	...
	return rpcEndpoint{IP: ip, UDP: uint16(addr.Port), RUDP: rudpPort, NAT: nat, NodeType: nodetype}
}
...
func (t *udp) nodeFromRPC(sender *net.UDPAddr, rn rpcNode) (*node, error) {
	...
	n := wrapNode(enode.NewV4(key, rn.IP, int(rn.RUDP), int(rn.UDP), uint16(rn.NAT), uint16(rn.NodeType)))
...
func nodeToRPC(n *node) rpcNode {
	...
	return rpcNode{ID: ekey, IP: n.IP(), UDP: uint16(n.UDP()), RUDP: uint16(n.RUDP()), NAT: n.NAT(), NodeType: n.Nodetype()}
}
...
func ListenUDP(c conn, ln *enode.LocalNode, cfg Config) (*Table, error) {
	...
	logging.Info("UDP listener up", "self", tab.self().String())
	return tab, nil
}
...
func (t *udp) ourEndpoint() rpcEndpoint {
	...
	return makeEndpoint(a, uint16(n.RUDP()), n.NAT(), n.Nodetype())
}
...
func (t *udp) sendPing(toid enode.ID, toaddr *net.UDPAddr, callback func()) <-chan error {
	req := &ping{
		Version:    4,
		From:       t.ourEndpoint(),
		To:         makeEndpoint(toaddr, 0, 0, 0), // TODO: maybe use known TCP port from DB
...
func (t *udp) findnode(toid enode.ID, toaddr *net.UDPAddr, target encPubkey) ([]*node, error) {
	...
	errc := t.pending(toid, neighborsPacket, func(r interface{}) bool {
		...
		for _, rn := range reply.Nodes {
			...
			if err != nil {
				logging.Trace("Invalid neighbor node received", "ip", rn.IP, "addr", toaddr, "err", err)
				continue
			}
...
func init() {
	...
	maxSizeNode := rpcNode{IP: make(net.IP, 16), UDP: ^uint16(0), RUDP: ^uint16(0), NAT: 0, NodeType: 0}
	for n := 0; ; n++ {
...
func (t *udp) write(toaddr *net.UDPAddr, what string, packet []byte) error {
	...
	logging.Trace(">> "+what, "addr", toaddr, "err", err)
	return err
}
...
func encodePacket(priv *ecdsa.PrivateKey, ptype byte, req interface{}) (packet, hash []byte, err error) {
	...
	if err := rlp.Encode(b, req); err != nil {
		logging.Error("Can't encode discv4 packet", "err", err)
		return nil, nil, err
	}
	packet = b.Bytes()
	sig, err := crypto.Sign(crypto.Keccak256(packet[headSize:]), priv)
	if err != nil {
		logging.Error("Can't sign discv4 packet", "err", err)
		return nil, nil, err
	}
...
func (t *udp) readLoop(unhandled chan<- ReadPacket) {
	...
	for {
		...
		if netutil.IsTemporaryError(err) {
			...
			logging.Debug("Temporary UDP read error", "err", err)
			continue
		} else if err != nil {
			// Shut down the loop for permament errors.
			logging.Debug("UDP read error", "err", err)
			return
		}
...
func (t *udp) handlePacket(from *net.UDPAddr, buf []byte) error {
	...
	if err != nil {
		logging.Debug("Bad discv4 packet", "addr", from, "err", err)
		return err
	}
	err = packet.handle(t, from, fromID, hash)
	logging.Trace("<< "+packet.name(), "addr", from, "err", err)
	return err
}
...
func (req *ping) handle(t *udp, from *net.UDPAddr, fromKey encPubkey, mac []byte) error {
	...
	t.send(from, pongPacket, &pong{
		To:         makeEndpoint(from, req.From.RUDP, t.self().NAT(), t.self().Nodetype()),
		ReplyTok:   mac,
		Expiration: uint64(time.Now().Add(expiration).Unix()),
	})
	n := wrapNode(enode.NewV4(key, from.IP, int(req.From.RUDP), from.Port, uint16(req.From.NAT), uint16(req.From.NodeType)))
...
func (req *findnode) handle(t *udp, from *net.UDPAddr, fromKey encPubkey, mac []byte) error {
	...
	t.tab.mutex.Lock()
	closest := t.tab.closest(target, bucketSize, req.NatRange, req.NodeType).entries
	t.tab.mutex.Unlock()
