...
//TestGetNAT tests encoding/decoding and setting/getting of the NAT key.
func TestGetSetNAT(t *testing.T) {
	nat := NAT(16)
	var r Record
	r.Set(nat)

	var nat2 NAT
	require.NoError(t, r.Load(&nat2))
	assert.Equal(t, nat, nat2)
}

// TestGetSetNodeRole tests encoding/decoding and setting/getting of the NodeType key.
func TestGetSetNodeType(t *testing.T) {
	nodeType := NODETYPE(8)
	var r Record
	r.Set(nodeType)

	var nodeType2 NODETYPE
	require.NoError(t, r.Load(&nodeType2))
	assert.Equal(t, nodeType, nodeType2)
	t.Logf("nodetype is:%s\n", nodeType2.ENRKey())
}
