...
func NewLocalNode(db *DB, key *ecdsa.PrivateKey, nat NATType, nodetype params.NodeType) *LocalNode {
	ln := &LocalNode{
	...
	enat := enr.WithEntry("nat", uint16(nat))
	enodetype := enr.WithEntry("nodetype", uint16(nodetype))
	ln.set(enat)
	ln.Set(enodetype)
	ln.invalidate()
	return ln
...
func (ln *LocalNode) NAT() NATType {
	return ln.nat
}

func (ln *LocalNode) NODETYPE() params.NodeType {
	return ln.nodeType
}
...
func (ln *LocalNode) sign() {
	...
	ln.cur.Store(n)
	logging.Info("New local node record", "seq", ln.seq, "id", n.ID(), "ip", n.IP(), "udp", n.UDP(), "rudp", n.RUDP(), "nat", n.NAT(), "node type", n.Nodetype())
}
