...
const (
	...
	NATFull                         //全锥形
	NATRestricted                   //ip限制
	NATPortRestricted               //ip and port 限制
	NATSymmetric                    //对称
	NATSymmetricUDPFirewall         //对称并且有防火墙
)

func init() {
	natStr = map[NATType]string{
		NATUnknown:              "Unexpected response from the nat server",
...
func GetNATTypeFromString(stype string) NATType {
	switch stype {
	case "Unknown":
		return NATUnknown
	case "None":
	...
	case "SymmetricUDPFirewall":
		return NATSymmetricUDPFirewall
...
// RUDP returns the RUDP port of the node
func (n *Node) RUDP() int {
	var port enr.RUDP
	n.Load(&port)
	return int(port)
}

//NAT returns the NAT type of the node.
func (n *Node) NAT() NATType {
	var nat enr.NAT
	n.Load(&nat)
	return NATType(nat)
}
...
func (n *Node) Nodetype() params.NodeType {
	var nodetype enr.NODETYPE
	n.Load(&nodetype)
	return params.NodeType(nodetype)
}
...
func (n *Node) URLv4() string {
	...
	if n.Incomplete() {
		...
		addr := net.TCPAddr{IP: n.IP(), Port: n.RUDP()}
