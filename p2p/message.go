...
func (msg *Msg) Hash() common.Hash {
	...
	key := bytes.NewBuffer(code)
	key.Write(payload)

	checkSum := crypto.Keccak256Hash(key.Bytes())

	msg.hash.Store(checkSum)
	return checkSum
}
...
func (p *MsgPipeRW) Addr() common.Address {
	return common.HexToAddress("0x0000001111")
}
...
// feed
func newMsgEventer(rw MsgReadWriter, feed *event.Feed, peer *Peer, proto string) *msgEventer {
	return &msgEventer{
		MsgReadWriter: rw,
		feed:          feed,
		peer:          peer,
...
func (ev *msgEventer) ReadMsg() (Msg, error) {
	...
	ev.feed.Send(&PeerEvent{
		...
		Peer:     ev.peer.ID(),
		Enode:    ev.peer.Node().String(),
		IP:       ev.peer.Node().IP().String(),
