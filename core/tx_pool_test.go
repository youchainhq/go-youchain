...
func (bc *testBlockChain) Processor() Processor {
	return bc.processor
}
...
func (bc *testBlockChain) StateAt(common.Hash, common.Hash, common.Hash) (*state.StateDB, error) {
...
func pricedTransaction(nonce uint64, gaslimit uint64, gasprice *big.Int, key *ecdsa.PrivateKey) *types.Transaction {
	tx, _ := types.SignTx(types.NewTransaction(nonce, common.Address{}, big.NewInt(100), gaslimit, gasprice, nil), types.MakeSigner(big.NewInt(0)), key)
	return tx
}

func setupTxPool() (*TxPool, *ecdsa.PrivateKey) {
	statedb, _ := state.New(common.Hash{}, common.Hash{}, common.Hash{}, state.NewDatabase(youdb.NewMemDatabase()))
	blockchain := &testBlockChain{statedb, 1000000, new(event.Feed), testProcessor()}
...
// testProcessor return a processor for test use.
func testProcessor() Processor {
	return &StateProcessor{txConverters: make(map[common.Address]TxConverter), defaultConverter: &DefaultConverter{}}
}
...
func deriveSender(tx *types.Transaction) (common.Address, error) {
	return types.Sender(types.MakeSigner(big.NewInt(0)), tx)
}
...
func (c *testChain) State() (*state.StateDB, error) {
	...
	stdb := c.statedb
	if *c.trigger {
		c.statedb, _ = state.New(common.Hash{}, common.Hash{}, common.Hash{}, state.NewDatabase(youdb.NewMemDatabase()))
		// simulate that the new head block included tx0 and tx1
		c.statedb.SetNonce(c.address, 2)
		c.statedb.SetBalance(c.address, new(big.Int).SetUint64(params.YOU))
		*c.trigger = false
	}
	return stdb, nil
}
...
func TestStateChangeDuringTransactionPoolReset(t *testing.T) {
	...
	statedb.SetBalance(address, new(big.Int).SetUint64(params.YOU))
	blockchain := &testChain{&testBlockChain{statedb, 1000000000, new(event.Feed), testProcessor()}, address, &trigger}
...
func TestTransactionNegativeValue(t *testing.T) {
	...
	tx, _ := types.SignTx(types.NewTransaction(0, common.Address{}, big.NewInt(-1), 100, big.NewInt(1), nil), types.MakeSigner(big.NewInt(0)), key)
	from, _ := deriveSender(tx)
	pool.currentState.AddBalance(from, big.NewInt(1))
...
func TestTransactionChainFork(t *testing.T) {
	...
	resetState := func() {
		...
		pool.chain = &testBlockChain{statedb, 1000000, new(event.Feed), testProcessor()}
		<-pool.requestReset(nil, nil)
	}
	resetState()
...
func testTransactionQueueGlobalLimiting(t *testing.T, nolocals bool) {
	...
	pool := NewTxPool(config, blockchain)
	defer pool.Stop()
...
func TestTransactionPendingMinimumAllowance(t *testing.T) {
	...
	config.GlobalSlots = 1024
	config.AccountQueue = 1024
	config.GlobalQueue = 2048
	config.GlobalSlots = 1024
...
func TestTransactionPoolStableUnderpricing(t *testing.T) {
	...
	createTx := config.GlobalSlots + config.GlobalQueue
	// Fill up the entire queue with the same transaction price points
	txs := types.Transactions{}
	for i := uint64(0); i < createTx; i++ {
		txs = append(txs, pricedTransaction(i, 100000, big.NewInt(1), keys[0]))
	}
	pool.AddRemotesSync(txs)

	pending, queued := pool.Stats()
	assert.Equal(t, int(createTx), pending)
	assert.Equal(t, int(0), queued)

	if err := validateEvents(events, int(createTx)); err != nil {
		t.Fatalf("original event firing failed: %v", err)
	}
	if err := validateTxPoolInternals(pool); err != nil {
		t.Fatalf("pool internal state corrupted: %v", err)
	...
	pending, queued = pool.Stats()
	if pending != int(createTx) {
		t.Fatalf("pending transactions mismatched: have %d, want %d", pending, createTx)
	}
	if queued != 0 {
		t.Fatalf("queued transactions mismatched: have %d, want %d", queued, 0)
	}
...
func testTransactionJournaling(t *testing.T, nolocals bool) {
	...
	logging.Info("pool stats", "pending", pending, "queued", queued)
