...
type HeaderChain struct {
	...
	// fields for light-client

	// the start header of a light-client.
	// All old headers between genesis and lightStartHeader may not stored locally.
	lightStartHeader atomic.Value
}
...
// into the chain. Hence, writing headers directly should only be done
...
func (hc *HeaderChain) WriteHeader(header *types.Header) error {
	...
	hc.SetCurrentHeader(header)
	metricsHeadHeaderGauge.Update(int64(number))
...
func (hc *HeaderChain) ValidateHeaderChain(chain []*types.Header, seals []bool) (int, error) {
	...
	for i := 1; i < len(chain); i++ {
		if chain[i].Number.Uint64() != chain[i-1].Number.Uint64()+1 || chain[i].ParentHash != chain[i-1].Hash() {
			...
			logging.Error("Non contiguous header insert", "number", chain[i].Number, "hash", chain[i].Hash(),
	...
	for i := range chain {
		...
		if hc.procInterrupt() {
			logging.Debug("Premature abort during headers verification")
...
func (hc *HeaderChain) InsertHeaderChain(chain []*types.Header, writeHeader WhCallback, start time.Time) (int, error) {
	...
	for i, header := range chain {
		...
		if hc.procInterrupt() {
			logging.Debug("Premature abort during headers import")
	...
	logging.Info("Imported new block headers", context...)
...
func (hc *HeaderChain) GetLightStartHeader() *types.Header {
	return hc.lightStartHeader.Load().(*types.Header)
}
func (hc *HeaderChain) SetLightStartHeader(head *types.Header) {
	rawdb.WriteLightStartHeaderHash(hc.chainDb, head.Hash())
	hc.lightStartHeader.Store(head)
}

func (hc *HeaderChain) storeLightStartHeader(head *types.Header) {
	hc.lightStartHeader.Store(head)
}
...
type (
	...
	UpdateHeadBlocksCallback func(youdb.Putter, *types.Header)

	// DeleteBlockContentCallback is a callback function that is called by SetHead
	// before each header is deleted.
	DeleteBlockContentCallback func(youdb.Deleter, common.Hash, uint64)
)
...
func (hc *HeaderChain) GetVldReader(valRoot common.Hash) (state.ValidatorReader, error) {
	return hc.parent.GetVldReader(valRoot)
}
